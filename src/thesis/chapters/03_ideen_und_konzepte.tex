...
Als Inhalt für die zu klassifizierenden Webseiten wird die Simple-English-Wikipedia verwendet, lokal in der Simulation über einen eigens entwickelten Webserver ausgeliefert.
Wikipedia liefert pro Artikel eine eigenständige HTML-Seite, was eine saubere Eins-zu-eins-Zuordnung zwischen Klasse und Inhalt ermöglicht, und ein lokaler Mirror umgeht den unter Shadow nicht vorgesehenen Internetzugriff der Simulation (technischer Aufbau in Abschnitt~\ref{sec:realisierung_zimsrv}).
...
Eine eigene Verkehrsmodellierung wird nicht entwickelt, da diese Modelle an PrivCount-Messungen kalibriert sind und damit eine validierte Grundlage für realistischen Mischverkehr bilden~\cite{mani2018understanding}.

\section{Methodische Vereinfachung: Pcap statt Cell-Trace}
\label{sec:konzepte_pcap}

Jansen und Wails verwenden für die Aufzeichnung eine gepatchte Tor-Version, die nicht öffentlich verfügbar ist~\cite{jansen2023data}.
...
Der Vorteil liegt darin, dass das \gls{pcap} dem Sichtfeld eines passiven Beobachters und damit dem üblichen \gls{wf}-Bedrohungsmodell entspricht~\cite{jansen2023data}.
...
Diese strikte Isolation ist eine Vereinfachung gegenüber dem realen Tor-Browser-Verhalten (First-Party-Bundling)~\cite{jansen2023data}, in der \gls{wf}-Literatur jedoch übliche Praxis und für die saubere Zuordnung von Verkehr zu Klasse notwendig.
