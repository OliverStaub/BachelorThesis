...
Im \texttt{schedule.json} wird jeder Besuch mit Startzeit und Zielport protokolliert, wodurch der Port als ganzzahliges, eindeutiges Klassen-Label dient.

Die Zuordnung von Paketen zu Besuchen erfolgt im Konvertierungsschritt über das Zeitfenster aus dem Schedule (Startzeit zuzüglich Besuchsintervall), der Port liefert anschliessend das Klassen-Label dieses Besuchs.
...
Klassenüberlagerungen innerhalb eines Fensters sind damit per Versuchsdesign ausgeschlossen, ohne dass eine Klassenmarkierung im Tor-Datenstrom selbst (wie bei Jansen und Wails durch eine gepatchte Tor-Version) notwendig wäre~\cite{jansen2023data}.
