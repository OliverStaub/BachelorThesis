...
Dieses Kapitel begründet, wie die in Kapitel~\ref{cha:stand_der_technik} beschriebenen Bausteine Tor, Website-Fingerprinting, \gls{circuitpadding} und \gls{shadow} zu einem Versuchsaufbau zusammengeführt werden.
...
Padding und Website-Fingerprinting wurden in der Forschungsliteratur untersucht, aber nicht in einer Shadow-Simulation systematisch gekoppelt.
...
Die primäre Untersuchung erfolgt im \gls{closedworld}, ergänzt um ein einzelnes \gls{openworld}-Experiment als Realitätsanker.
...
Emulationsbasierte Tor-Testumgebungen wie ExperimenTor und NetMirage verfolgen den Ansatz der Netzwerk-Emulation und nicht den für diese Arbeit benötigten Ansatz der reproduzierbaren, ereignisbasierten Simulation~\cite{neverenough-sec2021}.
Live-Tor-Messungen sind nicht reproduzierbar~\cite{jansen2023data}.
Shadow mit Tornettools erfüllt alle Anforderungen: unmodifizierter Tor-Code und deterministischer Ablauf~\cite{netsim-atc2022}, und Jansen und Wails belegen mit 86\,\% Cross-Dataset-Accuracy des CUMUL-Klassifikators die Eignung für \gls{wf}-Studien~\cite{jansen2023data}.
...
\gls{circuitpadding} wirkt zudem direkt auf die von \gls{df} genutzten Richtungsfeatures, was den Padding-Effekt besonders sichtbar macht.
Die Validierung von \gls{df} in Shadow durch Jansen und Wails (76\,\% bis 82\,\% Accuracy auf Live-Tor) sichert die Vergleichbarkeit mit bestehender Forschungsliteratur~\cite{jansen2023data}.

\section{Wahl des Szenarios}
\label{sec:konzepte_szenario}

Die primäre Untersuchung erfolgt im \gls{closedworld}, weil der Fokus auf dem isolierten Padding-Effekt liegt und nicht auf der Robustheit gegenüber unbekannten Seiten.
Die \gls{closedworld}-Accuracy ist zudem deutlich stabiler als die \gls{openworld}-Kennzahlen~\cite{jansen2023data}, was beobachtete Effekte klarer dem Padding zuordnen lässt.
Ergänzend wird ein \gls{openworld}-Experiment als Realitätsanker durchgeführt, ohne den Padding-Modus dort systematisch zu variieren.
...
\gls{tamaraw} ist nicht Bestandteil des regulären Tor-Daemons.
Im Gegensatz zum produktiv ausgerollten \gls{circuitpadding} plant das Tor-Projekt auch keine native Implementierung.
\gls{tamaraw} verzögert reale Zellen systematisch und erreicht in der ursprünglichen Auswertung rund 199\,\% Bandbreiten-Overhead bei zusätzlichem Latenz-Overhead~\cite{cai2014systematic}.
...
Um \gls{tamaraw} dennoch in einem Tor-Kontext zu untersuchen, wird das Verfahren als externes Hilfsprogramm zwischen Monitor und Bridge realisiert, das den Zellfluss vor dem Versand abfängt, gemäss \gls{tamaraw}-Regelwerk formt und anschliessend wieder an Tor übergibt.
...
Wikipedia liefert pro Artikel eine eigenständige HTML-Seite, was eine saubere Eins-zu-eins-Zuordnung zwischen Klasse und Inhalt ermöglicht.
Da in Shadow kein Internetzugriff möglich ist, wird ein lokaler Spiegel eingerichtet.


\section{Methodische Vereinfachung: \gls{pcap} statt Cell-Trace}
...
\gls{circuitpadding} wirkt direkt auf die Richtungsabfolge, weshalb der Effekt im Richtungssignal am deutlichsten sichtbar wird.
...
Die inhaltliche Zuordnung ist aus dem Monitor-\gls{pcap} allein nicht rekonstruierbar.
Das \gls{pcap} zeichnet ausschliesslich den verschlüsselten Verkehr zum Tor-Guard auf, weder Ziel-IP noch Zielport des angesprochenen Webservers sind sichtbar.
Genau dieser Umstand entspricht dem realistischen Angriffsmodell: der passiven beobachtenden Instanz zwischen Monitor und Guard fehlt eben jene Information.
...
Diese strikte Isolation ist eine Vereinfachung gegenüber dem realen Tor-Browser-Verhalten (First-Party-Bundling)~\cite{jansen2023data}, in der \gls{wf}-Forschungsliteratur jedoch übliche Praxis.
...
Die Tor-Topologie wird mit Tornettools auf 1\,\% des Live-Netzwerks skaliert~\cite{neverenough-sec2021}, was in der verwendeten Januar-2025-Topologie 79~Relais ergibt.
...
Die Reduktion bildet das Live-Netzwerk nur eingeschränkt ab, sodass sich das kleinere Netzwerk anders oder weniger realistisch verhalten kann und die absoluten Accuracy-Werte nicht unmittelbar auf das reale Netzwerk übertragbar sind.
Die methodischen Konsequenzen dieser Reduktion werden in Kapitel~\ref{cha:diskussion} aufgegriffen.