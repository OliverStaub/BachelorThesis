...
Stattdessen wird am Monitor eine fortlaufende \gls{pcap} Datei mit allen TCP-Paketen aufgezeichnet und im Nachgang in beschriftete Richtungssequenzen pro Seitenbesuch zerlegt.
...
Diese Zerlegung übernimmt ein eigens für die vorliegende Arbeit entwickeltes Python-Skript (vgl.\ Abschnitt~\ref{sec:realisierung_pcap_to_npz}), das das \gls{pcap} mit der \texttt{dpkt}-Bibliothek parst und die Pakete anhand der in der \texttt{schedule.json} protokollierten Besuchsfenster und der Port-zu-Klasse-Zuordnung (Abschnitt~\ref{sec:konzepte_port_class}) den jeweiligen Seiten zuweist.

Bei diesem Schritt gehen drei Arten von Information verloren.
Erstens kann ein TCP-Segment mehrere Tor-Zellen enthalten, weshalb die zellgenaue Auflösung des Cell-Trace nicht erreicht wird und der Padding-Effekt im \gls{pcap} tendenziell konservativer sichtbar ist.
Zweitens werden die Richtungssequenzen nach Sirinam et al.\ auf 5000~Werte normiert (Abschnitt~\ref{sec:konzepte_features}), wodurch Pakete besonders langer Besuche abgeschnitten werden.
Drittens werden Besuche mit zu wenigen Paketen als fehlgeschlagene Abrufe verworfen.

Trotz dieser Vereinfachungen bildet das \gls{pcap} ein realistisches Angriffsszenario gegen einen passiven Beobachter ab, dem ebenfalls keine internen Tor-Cell-Traces zur Verfügung stehen~\cite{jansen2023data}.

\section{Port-zu-Klasse-Zuordnung}
