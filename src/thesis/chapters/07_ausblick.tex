...
Eine Wiederholung jedes Hauptexperiments mit mindestens drei unabhängigen Seeds würde Mittelwerte, Standardabweichungen und Konfidenzintervalle pro Konfiguration ermöglichen.
...
Da die Laufzeit linear mit Seeds und Visits wächst und insbesondere die \gls{tamaraw}-\gls{openworld}-Läufe bereits an die Disk-Obergrenze stiessen, müsste zugleich auf das in Abschnitt~\ref{sec:eval_performance} skizzierte Cell-Trace-Logging umgestellt werden.
...
Eine Folgearbeit könnte statt der Wikipedia andere Websites in den \gls{shadow}-Container spiegeln, etwa das 100-URL-Set von Jansen und Wails~\cite{jansen2023data} oder einen statisch gespiegelten Auszug einer Alexa-Top-N-Liste.
...
Der eingesetzte \texttt{wget2}-Client bildet die wichtigste Vereinfachung gegenüber dem realen Tor-Browser-Betrieb.
...
Eine Folgearbeit könnte den Monitor-Client durch einen headless betriebenen Tor-Browser ersetzen, der über eine Steuerschnittstelle deterministisch eine vorgegebene Seitensequenz lädt, und damit JavaScript-Ausführung, dynamisches Nachladen und parallele Tabs abbilden.
...
Mit beiden Massnahmen liesse sich erstmals untersuchen, ob die hier gemessene Schutzwirkung von \gls{circuitpadding} mit wachsender Pfaddiversität abnimmt, gleich bleibt oder sich invertiert.
...
Eng verwandt mit \gls{wf}, aber mit eigenständigem praktischem Wert, ist die Frage, ob sich Tor-Verkehr mit einem fingerprinting-ähnlichen Verfahren überhaupt von gewöhnlichem Verkehr unterscheiden lässt.
Diese Unterscheidung ist die Grundlage staatlicher Zensur, etwa durch die Great Firewall in China, und der Grund, weshalb Tor pluggable transports wie obfs4 einsetzt, die den Verkehr verschleiern sollen~\cite{saputra2016detecting}.
