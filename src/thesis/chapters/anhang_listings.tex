...
Die von \texttt{generate-wf-config.py} erzeugte Datei \texttt{schedule.json} dient als Ground Truth für die spätere Zuordnung von \gls{pcap}-Abschnitten zu Seitenaufrufen (vgl.\ Abschnitt~\ref{sec:realisierung_generate_wf_config}).
...
Die von \texttt{generate-urls.py} erzeugte Datei \texttt{urls.txt} legt fest, welche Wikipedia-Seiten den \gls{monitorhost}s zugewiesen werden (vgl.\ Abschnitt~\ref{sec:realisierung_webserver}).
